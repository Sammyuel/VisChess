...
function* run(context, heroku) {
  ...
  if (git.inGitRepo()) {
    ...
    _((yield git.listRemotes())).filter(r => git.gitUrl(app) === r[1] || git.sshGitUrl(app) === r[1]).map(r => r[0]).uniq().forEach(git.rmRemote);
  }
}
...
module.exports = [Object.assign({ topic: 'apps', command: 'destroy' }, cmd), Object.assign({ hidden: true, topic: 'destroy' }, cmd), Object.assign({ hidden: true, topic: 'apps', command: 'delete' }, cmd)];