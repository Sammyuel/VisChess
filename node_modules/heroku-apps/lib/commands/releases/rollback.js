...
To undo, run: ${cli.color.cmd('heroku rollback v' + (latest.version - 1))}`);
  ...
  if (latest.output_stream_url) {
    ...
    yield output.Stream(latest.output_stream_url).catch(err => {
...
module.exports = [Object.assign({ topic: 'releases', command: 'rollback' }, cmd), Object.assign({ hidden: true, topic: 'rollback' }, cmd)];